...
class Event(BaseModel):
    ...
    def model_post_init(self, ctx: Any) -> None:
        if (num_chan := len(self.channels)) != (num_amps := len(self.amplitudes)):
            raise ValueError(
                f"The number of channels and amplitudes in this Event are not equal. len(channels) == {num_chan} and len(amplitudes) == {num_amps}"
            )
        return
...
class Detector(BaseRNGModel):
    ...
    def model_post_init(self, ctx: Any) -> None:
        ...
        if num_channel_widths > self.num_channels:
            raise ValueError(
                f"There are more channel widths {num_channel_widths} than there are channels {self.num_channels}."
            )
        if num_channel_widths < self.num_channels and num_channel_widths != 1:
            raise ValueError(
                f"There are less channel widths {num_channel_widths} than there are channels {self.num_channels}."
            )
        ...
        self._channel_center_positions: npt.NDArray[float] = np.arange(
            self.channel_start_pos, (self.num_channels + 1) * self.channel_spacing, self.channel_spacing
        )
        ...
        for channel in range(self.num_channels):
            ...
            self._channel_coverage[channel] = (center - channel_widths[channel], center + channel_widths[channel])
    ...
    def process_emission(self, emission: Emission, position: npt.NDArray[float]) -> Event:
        ...
        for channel in range(self.num_channels):
            ...
            if r >= coverage[0] and r < coverage[1]:
                rand_noise: float = self._channel_resolution_fC * (
                    np.sum(rng.random(self.signal_width)) - self.signal_width / 2
                )
